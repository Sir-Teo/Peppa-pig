...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        arguments = rest.map(lambda operand: scheme_eval(operand, env))
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        while parent_frame is not None:
            if symbol in parent_frame.bindings.keys():
                return parent_frame.bindings[symbol]
    ...
    def rebind(self, symbol, value):
        ...
        while parent_frame is not None:
            if symbol in parent_frame.bindings.keys():
                parent_frame.bindings[symbol] = value
                return
            parent_frame = parent_frame.parent
        # END PROBLEM 3
        raise SchemeError('unknown identifier: {0}'.format(symbol))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        #expression.second is a pair, stuck at this point for 302923 hours
        ...
        l = LambdaProcedure(formals,body,env)
        name = target.first
        env.define(name, l)
        return name
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while expressions.second is not nil:
            # forgot to use the function scheme_falsep/scheme_truep and stuck for 92839084 hours
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            if clause.second is nil or clause.first is nil:
                return test #return test instead of eval_all(clause, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    # they are nils at the beginning
    formal, value = nil, nil
    while bindings is not nil:      
        check_form(bindings.first,2,2)
        formal = Pair(bindings.first.first, formal)
        value = Pair(scheme_eval(bindings.first.second.first, env), value)
        bindings = bindings.second  
    check_formals(formal)
    let_frame = env.make_child_frame(formal, value)
    return let_frame
    # END PROBLEM 15
